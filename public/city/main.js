(function() {
  ...
  function buildings() {
    ...
    // Let the cube's matrix be located at the bottom
    geometry.applyMatrix(new THREE.Matrix4().makeTranslation(0, 0.5, 0));

    // Remove bottom face as it is never seen
    let uv = geometry.faceVertexUvs[0];
    geometry.faces.splice(6, 2);
    uv.splice(6, 2);

    // Set the top of the building to be the first pixel (white)
    uv[4][0].set(0, 0);
    uv[4][1].set(0, 0);
    uv[4][2].set(0, 0);
    uv[5][0].set(0, 0);
    uv[5][1].set(0, 0);
    uv[5][2].set(0, 0);
    // geometry.faceVertexUvs[0][1][3].set(0, 0);
